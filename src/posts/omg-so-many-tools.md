---
title: "OMG, so many tools!"
description: "These days, there are Tons of tools all fighting for the attention of developers. Productivity apps,..."
date: "2023-03-17"
...
It has markdown support (which is a must for me), a really nice UI, and an open API that's allowed for some pretty great community developed plugins. I personally have mine synced with my GitHub account so all my notes get committed to a private repo...no subscription fee at all.
...
I've bounced between 1 or 2 over the years, but am currently using [1Password](https://1password.com/) and I've been pretty happy when it so far. They have extensions or add-ons for different browsers that all work really well. Their UI is simple and easy to use. They even have a desktop app, which is nice for quick access if you don't already have your browser open.

So far I give 1Password a :thumbsup:, but it's only been a couple of months, so we'll see.
...
When I'm developing, I'm usually using [Brave](https://brave.com/) (but of course testing in all the other ones too...). Like so many browsers today, Brave is based on Chromium, but is focused on privacy. It "blocks the trackers & creepy ads on every website you visit". If you already use Chrome, Brave would not even be a stretch for you to switch to.

For personal use, I either use [Duck Duck Go](https://duckduckgo.com/mac) (DDG) or [Firefox](https://www.mozilla.org/en-US/firefox/new/). I've been using DDG on iOS for a long time now, and Firefox when I wasn't on a mobile device. But about a year ago, DDG released a browser for MacOS and now I go back and forth between the two.
...
### Code Editor
Like so many of us, I too have adopted [VSCode](https://code.visualstudio.com/). But I'm also a long time [Vim](https://www.vim.org/) user, and have recently started playing around with [Zed](https://zed.dev/). I haven't dug into Zed deep enough to know if I'll make a full switch, but it's been a really good experience so far!

#### Theme
In VSCode, I've been using the [Github Dark Default](https://marketplace.visualstudio.com/items?itemName=GitHub.github-vscode-theme) theme for a long time. In Vim, I use a custom theme I built a while ago called [Breeze](https://github.com/iamthe-Wraith/breeze) which is a fork of the [Dracula](https://github.com/dracula/vim) theme.
...
- [Better Comments](https://marketplace.visualstudio.com/items?itemName=aaron-bond.better-comments) - Really nice for adding some color highlighting to your comments.
- [Code Spell Checker](https://marketplace.visualstudio.com/items?itemName=streetsidesoftware.code-spell-checker) - I consider this one a must have. It'll let you know if you misspell a word in something like a variable or function name.
- [Colorize](https://marketplace.visualstudio.com/items?itemName=kamikillerto.vscode-colorize) - Great for visualizing the colors used in your css.
- [Draw.io Integration](https://marketplace.visualstudio.com/items?itemName=hediet.vscode-drawio) - If you're like me and you find flowcharts super helpful...this brings them to your editor, and stores the flowchart code in your project!
- [Eslint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint) - Like Mike Tyson, you don't have to ask who this celebrity is, or what it does.
- [GitHub Copilot](https://marketplace.visualstudio.com/items?itemName=GitHub.copilot) - AI at your fingertips.
- [GitLens](https://marketplace.visualstudio.com/items?itemName=eamodio.gitlens) - Another must have. Bring git into your editor and see who last modified a line, and what the context was around the change.
...
### Terminal
For the longest time, I only ever used the built in terminal that came with Mac. I played around with a few others, but always ended up coming back to the basic terminal...that was until I discovered [Warp](https://www.warp.dev/).

Warp is one of my favorite tools that I use today. It's super fast, allows you to split your window into multiple panes (something you used to have configure other tools like [tmux](https://github.com/tmux/tmux) to do), and comes with natural language AI baked in. Just tell the AI what you want to do, and it'll write the command for you...all without leaving your terminal.

But if you don't want to ditch your current terminal, that's okay. Before I found Warp, I had found another really cool tool called [Fig](https://fig.io/).
...
When I was looking for a VPN, I had 2 main concerns...E2E encryption, and no logs being stored. I bounced around between a couple differents options, but finally landed on [Proton VPN](https://protonvpn.com/). Part of the same suite of products mentioned in the [Email](#email) section above, it comes with security and privacy top of mind. Your data flows through an encrypted VPN tunnel, meaning your protected even over "unsafe" connections, they don't log any user behavior, and, as mentioned above, your privacy is protected by some of the strictest privacy laws on the planet.
...
One of the easiest ways to discover new stuff is just by listening to development related podcasts. 2 incredibly popular ones are [Syntax](https://syntax.fm/) and [JS Party](https://changelog.com/jsparty), and I would encourage everyone to check them out! The hosts are really entertaining, knowledgeable, and very easy to listen to for long periods of time...no dry, monotone, college professor lectures here!

If you have time to fit reading into your schedule, a few good sources are obviously here on [dev.to](https://dev.to). You can find some really good info here, and this isn't the first or last post on tools I'm sure. There are really great books out there as well. I just finished "Measure What Matters" by John Doerr and would highly recommend it. Lastly, there's a great browser extension/add-on called [daily.dev](https://daily.dev/) that I really enjoy. If you add that as your homepage, every time you open a new tab, you'll see a handful of articles that may allow you to discover new stuff as well.
